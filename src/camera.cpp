...
camera::camera(vec2i p_image_dimension, double p_focal_length) : m_image_dimension(p_image_dimension), m_focal_length(p_focal_length) {
	m_aspect_ratio = p_image_dimension[0] / p_image_dimension[1];
	...
	m_pixel_delta_u = viewportV() / static_cast<double>(m_image_dimension[1]); //now we know our viewport distance scaled to pixels. so we move half that distance across and down to have our points in the middle of what will become pixels
...
void camera::render() {
	...
	for(int row = 0; row < m_image_dimension[1]; row++) {
		for(int col = 0; col < m_image_dimension[0]; col++) {
			...
			color pixel_color = _ray.pixelColor();
