...
metal::metal(const color& p_albedo, double p_reflectance, double p_roughness) : m_albedo(p_albedo), m_reflectance(p_reflectance), m_roughness(p_roughness) {}
bool metal::reflect(const ray& p_ray, const hitRecord& p_record, vec3 &p_attenuation, ray& p_reflected) const {
	...
	vec3 random_dir = rng::vectorLambertian_fast(p_record.m_normal);
