...
sphere::sphere(const vec3f &p_center, double p_radius, material *p_material) : hittable(p_material), m_center(p_center), m_radius(p_radius) {} //im allowing for 0 and negative radii
...
bool sphere::intersect(const ray &p_ray, interval p_interval, hitRecord &p_record) const {
	...
	double h = dot(to_center, p_ray.m_direction);
