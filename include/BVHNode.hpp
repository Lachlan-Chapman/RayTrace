...
public:
	BVHNode() : BVHNode(BVHBounds(), 2, -1) {}

	explicit BVHNode(const BVHBounds &p_bounds, int p_childCount, int p_renderableId) : //without explicit you can do BVHNode p_bvh = p_obj where p_obj is of type hittable* because the operator= will think to convert p_obj to a BVHNode using this constructor
		m_bounds(p_bounds),
...
private:
	BVHBounds m_bounds; //copy of the always rect perfect fitting bounds of the renderable obj
};